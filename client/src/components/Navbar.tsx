  ...
  return (
    <nav className="bg-white dark:bg-gray-800 shadow">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <Link to={user.role === 'customer' ? `/home/customer/${user._id}` :`dashboard/restaurant/${user._id}`}>
          <div className="flex   items-center">
            
            <Utensils className="h-8 w-8 text-primary mr-2" />
            <span className="text-2xl font-bold text-gray-900 dark:text-white">PartyPalate</span>
          ...
          <div className='flex justify-between items-center'>
            {
              ...
              <ShoppingCart size={70} color='white' className="mr-2 h-6 w-6" />
              </div>
              </Link>
          ...
          <div className="flex items-center ml-2">
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                ...
                <DropdownMenuSeparator />
                <Link to={`/profile/${user.role === 'customer' ? 'customer' : 'restaurant'}/${user._id}`}>
                <DropdownMenuItem>
